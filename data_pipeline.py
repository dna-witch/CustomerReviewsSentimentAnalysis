...
class ETL_Pipeline():
    ...
    def preprocess(self):
        ...
        self.vocabulary = vectorizer.fit(self.tokens.apply(lambda x: ' '.join(x)))
    ...
    def encode(self, method='word2vec'):
        ...
        The default method is 'bow' (bag of words). Other options are 'tf-idf' and 'word2vec'.
        ...
        if method == 'bow':
            ...
            vectorizer = CountVectorizer()
            vectorized_data = vectorizer.fit_transform(self.tokens.apply(lambda x: ' '.join(x)))
            ...
            for doc in self.tokens:
                if doc:
                    ...
                    if doc_vectors:
                        vectorized_data.append(doc_vectors)
                    else:
                        print(f'No words in vocabulary for document: {doc}')
                else:
                    print(f'Empty document: {doc}')
            ...
            raise AttributeError("Invalid method argument. Please specify a valid encoding method: 'bow', 'tf-idf', or 'word2vec'.")
